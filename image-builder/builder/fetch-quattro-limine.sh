...
# Explicit Limine profile closure, independently signed by Arch Linux ARM.
# A rolling mirror may stop serving these bytes; never substitute a newer build.
destination=${1:?Usage: fetch-quattro-limine.sh DESTINATION}
builder_root=${BUILDER_ROOT:-/builder}
record="$builder_root/quattro-limine.json"
work=$(mktemp -d)
...
[[ $(grep '^depend = ' "$work/pkginfo") == "depend = $(jq -er '.required_dependency' "$record")" ]]
